...
例如$`S=0.10`$表示目标位置风速比参考位置高10%，$`S=-0.10`$则表示低10%。这个量依赖参考位置、风向、高度和气象状态，本身不提供场址长期年发电量（AEP）。
...
其中$`U_\infty`$表示未受尾流影响的来流，$`U_{wake}`$表示尾流区域风速。例如归一化速度亏损为0.20，表示尾流中的风速比未受扰动来流低20%。
...
**RANS（Reynolds-Averaged Navier–Stokes）**通过平均流方程和湍流闭合描述尾流，能够处理更复杂的地形和边界条件，计算成本高于工程模型。

**LES（Large-Eddy Simulation）**显式解析主要大尺度湍流结构，可研究尾流摆动、尾流相互作用和ABL耦合，计算成本进一步上升。
